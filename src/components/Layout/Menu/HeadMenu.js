      ...
      const menus = menuItems.map(item => {
        ...
        if (item.childs) {
          return (
            ...
            title={
              <SpanRipple>
                {newPKeys.length < 2 ?
                  <Icon className="top-nav-icon" type={item.menu_icon} />
                  :
        ...
        return (
          <MenuItem key={item.id} pathname={item.menu_url} pkeys={newPKeys}>
            <SpanRipple>
              {newPKeys.length < 2 ?
                <Tooltip arrowPointAtCenter placement="bottom" title={item.menu_name}>
                  {item.menu_icon ? <Icon className="top-nav-icon" type={item.menu_icon} /> : ""}
                </Tooltip>
              :
                item.menu_name
              }
